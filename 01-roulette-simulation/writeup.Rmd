...
```
  Then, we want to take a look at the way earnings change when parameter B, the starting budget, increases from $100 to $1,000.
```{r}
...
```
  In average, a player would play 204 times before one of the stopping conditions are satisfied and thus ends the game. The distribution is as follows:
```{r}
